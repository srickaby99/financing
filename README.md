...
- **Payment initiation — borrowers are never charged automatically** — the system is purely reactive. It waits for an external payment processor to notify it that a payment has settled (`POST /api/v1/webhooks/payments`). There is no code that looks at `next_due_date`, initiates a charge, or pushes anything to a processor. To actually collect repayments you must build: a scheduled job that identifies loans due today, submits charge requests to the processor, advances `next_due_date` after each successful payment, and handles failures (retries, late fees, delinquency status). This is the most critical missing piece before the system can service real loans.
